...
- 每檔必含 `license_status`（只收 `national_exam` 或 `school_official`，`unknown` 不得入庫）與 `source_url` 欄位（官方連結；來源待補時暫可留空字串 `""`，發布前補齊）。
...
merge → server 端（self-hosted runner / webhook，搆得到 localhost DB）：
  1. pull content
  2. sync：逐檔 transaction，四類內容各自 upsert——
     - questions(.md)：解析參照（school/subjects/departments）→ upsert Exam(校×年×管道)
  ...
  4. （選用）sync:prune：刪除已從 content repo 移除、且 DB metadata.sourcePath 可追蹤的
     題目 / 招生季 / 招生梯次·組 / 師資成員與孤兒考卷
...
- **萃取規格**（model-facing，區A+區B）：[tools/content-sync/PROMPT-admissions.md](../tools/content-sync/PROMPT-admissions.md)。
...
師資陣容是「**一校一系所一份 roster**」的策展資料（系所官網「專任師資」頁快照），顆粒度與考卷/招生不同，故獨立成樹、以 **`校/系所`** 為軸（師資屬（校,系所）級、非年度循環，故無年份段）。

```text
faculty/<school>/<dept>.yml   # 一系所一檔，內容為該系所完整 roster
```

- **契約**：`packages/shared/src/faculty-content.ts`（`FacultyYml`，物件皆 `.strict()` 抓拼錯）。檔級欄 `school`/`dept`/`as_of`（抓取日＝新鮮度錨點）/`source_url`（系所師資頁）；`members[]` 每筆以 **`name`（中文姓名）為系內身分鍵**，另有 `slug`/`name_en`/`title`（職級）/`lab`/`homepage`/`source_url`/`research_areas[]`/`degrees[]`（學歷）/`theses[]`（論文佐證，`role`＝`advised` 指導｜`authored` 自著）。
- **參照解析**：`school`/`dept` 為 seed slug（`schools.seed`），缺則 red——與 questions/admissions 同規則，不另建 map。
- **路徑↔內容一致**：檔內 `school`/`dept` 必須等於路徑段（`faculty/<school>/<dept>.yml`）。
- **sync**：`syncFaculty` 以 `(department, name)` upsert `FacultyMember`；`theses`/`degrees` 無自然鍵故全量重建。**檔即該系所完整 roster——每次 sync 會剔除檔中已不列的成員**（檔內移除即時反映，不必等 file-level prune）。
- **coverage 規劃輔助**（唯讀、無 DB）：`pnpm --filter @prograds/content-sync report-coverage <faculty-dir> [--gaps] [--md]`——對照 seed 系所清單列出已建/未建與每檔欄位完整度，導引補全順序。
- **萃取規格**（model-facing）：[tools/content-sync/PROMPT-faculty.md](../tools/content-sync/PROMPT-faculty.md)。
