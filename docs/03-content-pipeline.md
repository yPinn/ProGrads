...
**repo 分離（合規關鍵）**：程式碼為公開 OSS；題庫內容（含官方題幹）放**獨立私有 repo / submodule**。
...
```text
content/
  questions/<track>/<school>/<department>/<year>/<exam-subject>[/<group>]/<qNN>.md
  schedules/<year>/<school>.md
  stats/<track>/<school>.md
```

- 路徑段一律用 **slug**（穩定、無 CJK 路徑問題）；`<exam-subject>` 用卷的 slug。
- `<group>` 目錄層**僅在有組別時存在**（如電機所甲/乙/丙組）；無組別則省略。
- `admission_type` **預設 `exam`（考試入學）**，不進路徑；推甄無考古題，極少數例外才於 frontmatter 明寫。
- 例：`content/questions/computer-science/ntu/csie/2025/data-structures-algorithms/q03.md`
...
- **其餘用全字 kebab-case**：`computer-science`(track)、`data-structures`(subject)、`science-engineering`(category)、`data-structures-algorithms`(exam-subject)。
...
- 首次可由路徑推導預設值：`<school>-<department>-<year>-<exam-subject>[-<group>]-<qNN>`
  （例 `ntu-csie-2025-data-structures-algorithms-q03`）。
...
```markdown
---
question_id: ntu-csie-2025-data-structures-algorithms-q03 # 釘選，唯一鍵
exam_subject: 資料結構與演算法 # 卷的顯示名（ExamSubject.name；合科卷可多 subject）
subjects: [algorithms] # granular 練習標記（question_subject），用 slug
...
選擇題 `## 選項` 約定（**MC 第一刀延後**，先支援 essay/calc/proof）：
...
- sync 腳本（`tools/`）：**只做 Tier1/Tier2 essay 類** + 參照解析 + exam/exam_subject upsert + 收尾 reconcile。
- questions 讀取 API。
- **先不做**：Tier0 raw / blob store、整卷匯出、MC、knowledge_points、孤兒清理（各自後續一刀）。
