...
> **落地狀態（現況）**：本節為三層擁有者的最終設計。契約與 schema 已先行落地，內容 sync importer 尚未實作：
>
...
> - 已落地，區B importer：`tools/content-sync` 讀 `admissions/<year>/<school>/departments.yml`，upsert `AdmissionGroup`（content-derived，單一事實來源）+ `AdmissionRound`（逐年事實）+ 重建 `AdmissionRoundPaper`(+`Subject`) 與派生的 `AdmissionRoundSubject`（`src/admissions.ts`）。
> - 招生 API 現況：`/schedules` 讀校級 `AdmissionSeasonEvent`（content 來源；行事曆為校級，不含系所/組）。`/admissions`（組別/梯次）讀 `AdmissionGroup` / `AdmissionRound`，全數來自 `departments.yml`（`seed` 不再灌招生資料，已退役 `admission.seed.ts`）。`AdmissionRoundEvent`(round 級事件) 自 `/schedules` 切校級後僅 `/admissions` 仍用,後續評估改以 `AdmissionRound.interviewAt` 表組級面試、退役 round 事件。
>
...
下列表/欄已建（命名以實際 schema 為準）；剩餘缺口僅 content 到 DB 的 importer（見上節落地狀態）。
