...
- 已完成：`apps/api`：`taxonomy / schools / exams` 讀取 API（Fastify、Zod 契約、Swagger、統一錯誤信封）。
- 已完成：`tools/content-sync`：ProGrads-content → Postgres 同步（含合科卷 reconcile、MC 選擇題）。
- 已完成：`apps/api`：questions 讀取 API（含 choices）。
