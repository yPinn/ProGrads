...
- **單一歸屬**：每個主題只在一處完整說明，其餘文件連結、不重述（歸屬對照見 [06-decisions.md](06-decisions.md) D21）。
- **參考層**（`00`–`05`、`08`）：描述系統現況，程式碼改了才跟著改，可用完整敘述。
- **日誌層**（`06` ADR、`09` roadmap）：描述會被推翻的判斷，條列優先、why 一句話帶過。
- **草稿/未實作功能**：不寫成完整案例，內容精簡或移出正文。
