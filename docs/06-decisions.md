...
## D20. `AdmissionRound.quota`（考試入學）是簡章公告時的名額下限，非流用後定案值

- **決策**：`departments.yml` 抽自簡章的 `quota` 照舊直接存入 `AdmissionRound.quota`，**不**額外做「名額流用」
  修正管線；文件中註明此為已知限制，不視為資料錯誤。
- **背景**：台灣碩士班招生慣例為「甄試（推甄）先辦、考試入學後辦」；甄試備取遞補後若仍有缺額，該名額會
  「流用」併入考試入學，使考試入學實際可用名額只增不減。以 NTU（「碩士班甄試名額流用暨流用後碩招可口試
  名額異動一覽表」）、NTUT（「碩士班甄試備取生遞補後仍有缺額時，其招生名額得流用至碩士一般招生」）、NCKU
  （「其餘各學系所遞補作業⋯期限截止後不再遞補，其缺額併入一般招生考試補足」）三校官方簡章／公告互相佐證，
  確認方向一致、非個案。
- **115 學年度三校完整時序核實**（2026-07-23，逐校官方「重要日程表」原文核對）：

  | 階段                                              | NTHU                        | NTU                  | NYCU                          |
  | ------------------------------------------------- | --------------------------- | -------------------- | ----------------------------- |
  | 甄試報名                                          | 2025-10-08~10-14            | 2025-10-01~10-08     | ~10 月中旬前（推算）          |
  | 甄試放榜                                          | 3 梯：11-07/11-14/11-21     | 2 梯：11-05/11-20    | 2 梯：11-07/11-20             |
  | **甄試備取遞補截止（流用日）**                    | **2026-01-30**              | **2026-01-22 12:00** | **2026-02-23**                |
  | 考試報名（`admission-stats/` 報名人數表所在窗口） | 2025-12-02~12-09            | 2025-12-02~12-09     | 2025-12-09~12-16              |
  | 考試筆試                                          | 2026-02-04~05               | 2026-02-01~02        | 2026-02-04~06                 |
  | 考試放榜                                          | 03-06（初試）/03-27（複試） | 03-09/03-23          | 03-06（第1梯）/03-24（第2梯） |

  **NTHU／NTU**：流用截止（01-30／01-22）都**早於**考試筆試——流用在筆試前就定案，考試入學從筆試當天起用
  的已是流用後名額。**NYCU 是例外**：流用作業一路到 **02-23**，**晚於**考試筆試（02-04~06）——考生應試時
  還不知道最終流用後名額，流用結果要到放榜（03-06）前才定案。三校皆同：**考試報名一律發生在流用截止之
  前**，考生報名當下永遠不知道確切最終名額。

- **理由**：簡章公告時間點早於甄試放榜，此時的 `quota` 必然是「流用前」數字；但因流用只單向增加考試入學
  名額（甄試 → 考試，不會反向），該數字是**下限、不是錯誤值**——用它算出的錄取率是保守（偏高）估計，方向
  已知、可解讀，不需要另建「流用表」抽取管線去追一個持續變動、且對錄取率判斷影響有限的修正值。
