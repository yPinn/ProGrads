...
| 類型                         | 渲染                      | 位置                  | SEO  |
| ---------------------------- | ------------------------- | --------------------- | ---- |
| 考題庫 / 標準解答 / 報名情報 | runtime SSR + CF 邊緣快取 | CF Pages 邊緣         | 需要 |
| 做題記錄 / 弱點儀表板        | client-side               | CF（呼叫 server API） | 不需 |
| AI 追問 / 提醒訂閱           | server API                | server                | —    |

- **CF Pages**：Nuxt（`nitro preset: cloudflare-pages`），hybrid 渲染——內容頁走 runtime SSR、
  `routeRules` 設 `Cache-Control`（SWR）在邊緣快取回應，動態頁走 client。內容頁的資料抓取用
  `useApiQuery`（`@tanstack/vue-query` 包一層 `onServerPrefetch`）在 server 端預取並
  dehydrate 進 payload，client hydrate 續用同一 cache，故 SSR 回應直接帶內容而非空殼。理由與
  取捨見 D4。
...
對齊資料層，逐一照上述範本擴充（已做：`taxonomy / schools / exams / questions / admissions / faculty`）：
